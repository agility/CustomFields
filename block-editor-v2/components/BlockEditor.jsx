...
const BlockEditor = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      ...
      function (e) {
        ...
        if (e.data.type === "setInitialProps") {
          console.log(
            "Block Editor => Auth, fieldValue received from Agility CMS, setting up editor..."
          );
          auth = e.data.message.auth;
          custom = e.data.message.custom;
          fieldValue = e.data.message.fieldValue
            ? JSON.parse(e.data.message.fieldValue)
            : null;
          editor = setupEditor(
            auth,
            height,
    ...
    if (window.parent) {
      console.log(
        "Block Editor => Notifying CMS this field is ready to receive messages..."
      );
  ...
  return (
    <div style={{ background: "#fff", padding: "0 10px" }}>
      ...
      <div id="editorjs" ref={containerRef}></div>
    </div>
  );
};
...
const setupEditor = (
  ...
  const editor = new EditorJS({
    ...
    placeholder:
      "📝 Enter text, paste images/embed urls, or select a block to add here...",
    tools: {
      table: Table,
      paragraph: {
        class: Paragraph,
